...
class SuctionNetDataset(Dataset):
    ...
    def augment(self, img, depth, score, center_map):
        ...
        depth = cv2.warpAffine(depth, trans_input, (input_w, input_h), flags=cv2.INTER_LINEAR)
        score = cv2.warpAffine(score, trans_input, (input_w, input_h), flags=cv2.INTER_NEAREST)
        center_map = cv2.warpAffine(center_map, trans_input, (input_w, input_h), flags=cv2.INTER_NEAREST)
